...
- **Night mode** (`Night`/`Day` button in the nav, `localStorage` `tide-breath-night`): `:root[data-theme="night"]` swaps tokens to dim red on near-black, and turns the whole `#app` 90° with a CSS transform when the phone is portrait (a PWA can't force rotation, and rotation lock is usually on at night); a physically landscape phone just gets the two-column layout (circle left, guide/settings right). Manifest orientation is `any`.
